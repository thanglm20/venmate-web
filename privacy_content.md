# CHÍNH SÁCH BẢO MẬT
*Ngày cập nhật cuối cùng: 15 tháng 04 năm 2026*

Chào mừng bạn đến với **Venmate**. Chúng tôi cam kết bảo vệ quyền riêng tư của bạn. Chính sách này giải thích cách chúng tôi thu thập, sử dụng và bảo vệ thông tin của bạn khi bạn sử dụng ứng dụng di động **Venmate** và các dịch vụ liên quan.

### 1. Cam kết chung
 
**Venmate** cam kết bảo vệ và tôn trọng quyền riêng tư của người sử dụng (“Người dùng"). Chính sách Quyền riêng tư này giải thích cách chúng tôi thu thập, xử lý, sử dụng và bảo vệ dữ liệu cá nhân của bạn khi bạn truy cập và sử dụng Ứng dụng và Dịch vụ của chúng tôi.
 
Các thay đổi, cập nhật hoặc bổ sung đối với Chính sách này sẽ được đăng tải trên Ứng dụng. Việc bạn tiếp tục sử dụng Dịch vụ sau khi các thay đổi có hiệu lực sẽ được hiểu là bạn đã chấp nhận các điều khoản mới.

### 2. Thông tin chúng tôi thu thập
Để cung cấp dịch vụ kết nối đối tác thể thao tốt nhất, chúng tôi thu thập các loại dữ liệu sau:

* **Thông tin tài khoản:** Tên, email, số điện thoại, giới tính, ngày sinh, mật khẩu khi bạn đăng ký tài khoản **Venmate** hoặc đăng kí bằng Google Login.
* **Thông tin hồ sơ:** Ảnh đại diện (sử dụng Camera/Thư viện ảnh), sở thích môn thể thao (Cầu lông, Pickleball, Bóng đá...) và các thông tin bạn tự nguyện đăng tải lên ứng dụng.
* **Xác thực Google (Google Login):** Chúng tôi thu thập địa chỉ email, tên hiển thị và ảnh đại diện từ tài khoản Google của bạn để tạo hồ sơ người dùng và xác minh danh tính.
* **Dữ liệu vị trí (Location):** Dữ liệu định vị được thu thập qua GPS hoặc địa chỉ IP (nếu bạn cho phép) để hiển thị các đối tác và sân tập gần bạn nhất.
* **Dữ liệu kỹ thuật:** Địa chỉ IP, loại thiết bị, hệ điều hành, ID thiết bị (FCM Token) sẽ được chúng tôi thu thập để phục vụ mục đích định danh tài khoản, nâng cao bảo mật, chất lượng dịch vụ.
* **Cookies:** Hoặc các công nghệ theo dõi tương tự nhằm cá nhân hóa trải nghiệm và thu thập số liệu thông kê giúp chúng tôi cải thiện dịch vụ
* **Ngoài ra:** Chúng tôi có thể thu thập thông tin từ các bên thứ ban (đối tác, nhà cung cấp dịch vụ, mạng xã hội, ...) nếu bạn cung cấp và liên kết với ứng dụng.

### 3. Cách chúng tôi sử dụng thông tin
Chúng tôi sử dụng thông tin của bạn để:
* Xác thực người dùng và bảo mật tài khoản, hỗ trợ quá trình đăng kí, đăng nhập, quản lý tài khoản.
* Gợi ý những người bạn tập phù hợp dựa trên vị trí và sở thích.
* Giúp đối tác, bạn tập, người chơi có thể liên lạc khi bạn đăng kí tham gia và tạo các trận đấu thể thao.
* Tính năng Chat và WebSockets, tin nhắn được truyền qua giao thức bảo mật (WSS) để đảm bảo không bị can thiệp bởi bên thứ ba..
* Gửi thông báo đẩy (Push Notifications) về các trận đấu hoặc tin nhắn mới thông qua dịch vụ thông báo Google Cloud Messaging.
* Phân tích và cải thiện chất lượng dịch vụ (fix bug, tối ưu hóa giao diện).
* Tuân thủ pháp luật, xử lý dữ liệu để đáp ứng các nghĩa vụ pháp lý, bảo về quyền lợi hợp pháp của chúng và người dùng.

### 4. Chia sẻ thông tin
Chúng tôi **không** chia sẻ dữ liệu cá nhân của bạn cho bên thứ ba. Thông tin chỉ được chia sẻ trong các trường hợp:
* **Với người dùng khác:** Tên và hồ sơ thể thao của bạn sẽ hiển thị công khai để người khác có thể tìm thấy và kết nối.
* **Yêu cầu pháp lý:** Khi có yêu cầu từ cơ quan chức năng theo quy định của pháp luật Việt Nam.

### 5. Lưu trữ và Bảo mật
* **Máy chủ:** Dữ liệu cá nhân của bạn được lưu trữ trên máy chủ bảo mật của chúng tôi hoặc của đối tác được ủy quyền (EC2 Amazon), mọi thông tin được tuân thủ quy định pháp luật hiện hành.
* **Mã hóa:** Toàn bộ dữ liệu truyền tải giữa ứng dụng và máy chủ đều được mã hóa qua để tránh hacker có thể trích xuất thông tin.
* **Bảo mật:** Chúng tôi áp dụng các biện pháp bảo mật, mã hóa đường truyền và tường lửa để ngăn chặn truy cập trái phép.

### 6. Quyền của người dùng
Bạn có quyền:
* Truy cập và cập nhật thông tin cá nhân trong phần Cài đặt hồ sơ.
* Yêu cầu xóa tài khoản và toàn bộ dữ liệu liên quan.
* Thu hồi quyền truy cập Vị trí hoặc Camera bất kỳ lúc nào trong cài đặt điện thoại.

### 7. Thông tin liên hệ
Nếu có bất kỳ câu hỏi nào về chính sách này, vui lòng liên hệ:
* **Email:** venmate.support@gmail.com
* **Website:** [https://venmate.com.vn](https://venmate.com.vn)
* **Điện thoại:** 0975875811 

Phiên bản Chính sách Quyền riêng tư này được soạn thảo nhằm đảm bảo tuân thủ pháp luật Việt Nam về bảo vệ dữ liệu cá nhân. Bằng việc sử dụng Ứng dụng **Venmate**, bạn xác nhận đã đọc, hiểu và đồng ý với các điều khoản được nêu trong Chính sách Quyền riêng tư này.